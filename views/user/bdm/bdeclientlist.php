...
<head>
    ...
    //Client List ajax starts
        ...
        function bdeclient_fetch_List(){
                count++;
                $("#ajaxButton").prop("disabled", "true");
                $.ajax({
                        type: 'post',
                        url: '<?php echo BASEURL; ?>actions/bdm/performfetchclientlistforbde.php',
                        data: {
                        bdeempid : "<?php echo $_GET['bdeid']; ?>",
                        offsetVal: offset
                        },
                        success:function(response)
                        {
                            ...
                            for(var i=0; i<response.length; i++) 
                            {
                                ...
                                clientListBuilder += "<td align='center'><button type='button' class='btn btn-success btn-sm action-btn' title='Company List''  onclick='showClientCompany(" + response[i].company + ")' ><span class='glyphicon glyphicon-home'></span></button></td>";
                                
                                if((response[i].status=="Send Proposal")||(response[i].status=="New")) 
                                {
                                    clientListBuilder += "<td><a id='link' href='email_send.php?empid=<?php echo $_GET['bdeid'];?>&source=bdeclient&id=" + response[i].id + "'><p style='color:green;'> Send Proposal </p></a></td>";                          
                                }
                                else
                                {
    ...
    <div class="content-view">
        <div class="container-fluid">
            ...
            <?php if ($_SESSION['role'] == "BDM") : ?>
             ...
             <!-- Display Client List -->
             <?php
                 $bde_emp_id=$_GET['bdeid']; 
                 // echo $bde_emp_id;
                  $bdesql="select user_name from users where user_emp_id='$bde_emp_id'";
                  $bderesult=mysqli_query($conn,$bdesql);
                  $bderow=mysqli_fetch_object($bderesult);
                  $bdename=$bderow->user_name;
             ?>
             <a href="bdelist.php"><button type="button" class="btn btn-primary"><span class="glyphicon glyphicon-chevron-left"></span>Go Back</button></a>
             <h3 class="text-center">  Client Contact List of BDE: "<?php echo $bdename;?>"  </h3>
